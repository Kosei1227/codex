...
- Prefer these skill routes when applicable:
  - `$repo-context-bootstrap`: unfamiliar repo, module, workflow, or validation path.
  - `$solution-ai-engineer`: ambiguous product work, architecture-sensitive work, or end-to-end engineering tasks that need intent, design, implementation, validation, and reporting.
  - `$implement-and-validate`: direct implementation requests where Codex should own the edit and verification loop.
  - `$systematic-debugging`: bugs, regressions, flaky behavior, incorrect behavior, or unexplained failures.
  - `$experiment-driven-development`: unclear implementation paths, risky refactors, performance work, or tasks that need evidence from small trials.
  - `$long-horizon-development`: multi-hour, overnight, phase-completion, migration, or artifact-heavy work.
  - `$overnight-engineer`: unattended overnight coding, scheduled automations, `codex exec` runs, isolated worktree implementation, validation-backed checkpoints, PR-only handoff, or morning reports for work done while the user is away.
  - `$llm-integration-engineering`: runtime product systems that include LLM calls, RAG, tool use, LLM judges, semantic validation, planners, or agent workflows.
  - `$knowledge-graph-navigation`: architecture, cross-module relationship, design-rationale, and existing Graphify graph navigation tasks where graph output is only a candidate map and source files remain authoritative.
  - `$architecture-quality`: non-trivial code structure, feature design, refactoring, UI component systems, service boundaries, domain logic, adapters, dependency inversion, Atomic Design, SOLID, Hexagonal Architecture, Clean Architecture, Layered Architecture, responsibility separation, testability, or change-resistant design.
